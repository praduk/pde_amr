...
could a vector, for example in a dot product (divergence) or even matrix-vector product,
I introduce the symbols $\rdel$ and $\ldel$ and give precise meaning to them.
\end{rk}

\begin{df}[The right del operator $\rdel$]
    Let $g:\R^n\to \R^m$ maps $a \to g(a)$.
    The right del operator $\rdel_r$ is as a vector of partial differential operators,
    \begin{equation}
        \rdel_r = \begin{bmatrix}
            \pd{}{r_0} \\
            \pd{}{r_1} \\
            \dots
            \pd{}{r_n}
...
\begin{align}
    \alpha \tp{\left(\pd{f}{a}\right)} &= \alpha \nabla_a f = f \ldel_a \alpha = \tp{\left(\pd{f}{a}\right)} \alpha \\
